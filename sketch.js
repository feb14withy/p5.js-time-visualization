...
function setup() {
  ...
  for(let i = 0; i < maxRings; i++)
  {
    ...
    var randomMaxRadius = random(50, 600);
    ringObjectsArray[i] = createNewRing(ringGraphicsObject, random(200, width), height/2, 0, randomRingColour, randomMaxRadius);
...
function draw() {
  ...
  secondClosestNote = ringObjectsArray[4][3] * 2;
  highestClosestNote = ringObjectsArray[6][3] * 5;
  ...
  for(let i = 0; i < ringObjectsArray.length; i++)
  {
    ...
    {
      ringObjectsArray[i][3] += ALL_RINGS_GROWTH_FACTOR;
    }

    ringObjectsArray[i][1] -= ALL_RINGS_MOVEMENT_FACTOR;
...
function createNewRing(graphicsObject, xPosition, yPosition, ringRadius, ringColour, maxRingRadius)
{
  let newRingArray = [graphicsObject, xPosition, yPosition, ringRadius, ringColour, maxRingRadius];
  return newRingArray;
}
